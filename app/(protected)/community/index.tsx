	...
	const renderWatchContent = () => (
		<View className="px-6 py-4">
			...
			<BottomSheetModal visible={modalVisible} onClose={() => { setModalVisible(false) }}>
				<FlatList data={barangayCrimes.get(selectedBarangay!)} renderItem={(it) => {
					const crime = it.item
					return (
					...
					<View className="flex-row space-x-2">
						{forumCategories.map((category) => (
							<TouchableOpacity
								...
								className={`rounded-lg px-4 py-2 ${activeForumCategory === category ? 'bg-yellow-500' : 'bg-gray-200'
									}`}>
								<Text
									className={`font-medium ${activeForumCategory === category ? 'text-slate-900' : 'text-slate-600'
										}`}>
									{category}
								</Text>
							</TouchableOpacity>
		...
		<View className="flex-1 bg-white">
			...
			<View className="bg-gray-100 px-4 py-3">
				<View className="flex-row justify-between">
					<TouchableOpacity
						className={`mr-2 flex-1 items-center rounded-lg px-2 py-3 ${activeTab === 'Watch' ? 'bg-yellow-500' : 'bg-gray-200'
							}`}
						onPress={() => handleTabChange('Watch')}>
						<Text
							className={`text-sm font-semibold ${activeTab === 'Watch' ? 'text-slate-900' : 'text-slate-600'
								}`}>
							Watch
						</Text>
					</TouchableOpacity>
					<TouchableOpacity
						className={`mx-1 flex-1 items-center rounded-lg px-2 py-3 ${activeTab === 'Resources' ? 'bg-yellow-500' : 'bg-gray-200'
							}`}
						onPress={() => handleTabChange('Resources')}>
						<Text
							className={`text-sm font-semibold ${activeTab === 'Resources' ? 'text-slate-900' : 'text-slate-600'
								}`}>
							Resources
						</Text>
					</TouchableOpacity>
					<TouchableOpacity
						className={`ml-2 flex-1 items-center rounded-lg px-2 py-3 ${activeTab === 'Forums' ? 'bg-yellow-500' : 'bg-gray-200'
							}`}
						onPress={() => handleTabChange('Forums')}>
						<Text
							className={`text-sm font-semibold ${activeTab === 'Forums' ? 'text-slate-900' : 'text-slate-600'
								}`}>
							Forums
						</Text>
					</TouchableOpacity>

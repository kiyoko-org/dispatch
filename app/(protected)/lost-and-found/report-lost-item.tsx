...
export default function ReportLostItemScreen() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.last_seen_date) errors.last_seen_date = 'Date is required';
    if (!formData.last_seen_time) errors.last_seen_time = 'Time is required';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Report Submitted',
        'Your lost item report has been submitted successfully. You will receive a case ID shortly.',
        [
          {
            text: 'OK',
            onPress: () => router.back()
  ...
  const updateFormData = (updates: Partial<LostItemFormData>) => {
    ...
    <View className="space-y-5">
      ...
      <Card className="mb-5">
        ...
        <View className="space-y-4">
          ...
          <View>
            <Text className="mb-2 font-medium text-slate-700">
              Last Seen Date <Text className="text-red-600">*</Text>
            </Text>
            <TouchableOpacity
              onPress={() => openDropdown('date')}
              className="mb-3 flex-row items-center justify-between rounded-lg border border-gray-300 bg-white px-4 py-3">
              <Text className={formData.last_seen_date ? 'text-slate-900' : 'text-gray-500'}>
                {formData.last_seen_date || 'mm/dd/yyyy'}
              </Text>
              <Ionicons name="calendar" size={20} color="#64748B" />
            </TouchableOpacity>
            {validationErrors.last_seen_date && (
              <Text className="mt-1 mb-3 text-sm text-red-600">{validationErrors.last_seen_date}</Text>
            )}
          </View>
          ...
          <View>
            <Text className="mb-2 font-medium text-slate-700">
              Last Seen Time <Text className="text-red-600">*</Text>
            </Text>
            <TouchableOpacity
              onPress={() => openDropdown('time')}
              className="mb-3 flex-row items-center justify-between rounded-lg border border-gray-300 bg-white px-4 py-3">
              <Text className={formData.last_seen_time ? 'text-slate-900' : 'text-gray-500'}>
                {formData.last_seen_time || 'Select time'}
              </Text>
              <Text className="text-gray-400">▼</Text>
            </TouchableOpacity>
            {validationErrors.last_seen_time && (
              <Text className="mt-1 mb-3 text-sm text-red-600">{validationErrors.last_seen_time}</Text>
            )}
          </View>
      ...
      <Card className="mb-5">
        ...
        <View className="space-y-4">
          ...
          {/* Reward Amount */}
          <View>
            <Text className="mb-2 font-medium text-slate-700">Reward Amount (Optional)</Text>
            <TextInput
              placeholder="P0.00"
              value={formData.reward_amount}
              onChangeText={(value) => updateFormData({ reward_amount: value })}
      ...
      <Card className="mb-5">
        ...
        <View>
          <Text className="mb-2 font-medium text-slate-700">Add photos of the lost item</Text>
  ...
  return (
    <View className="flex-1 bg-gray-50">
      ...
      <DatePicker
        ...
        onSelectDate={(dateString) => updateFormData({ last_seen_date: dateString })}
        initialDate={formData.last_seen_date}
      />

      <TimePicker
        isVisible={showTimePicker}
        onClose={() => closeDropdown('time')}
        onSelectTime={(timeString) => updateFormData({ last_seen_time: timeString })}

...
export default function EmergencyScreen() {
	...
	const makeCall = async (
		...
		console.log('Logging emergency call to:', number, callerNumber, callType, locationLat, locationLng, outcome);
		try {
			const resp = await emergencyCallService.logEmergencyCall(
				number,
				callerNumber,
				callType,
				locationLat,
			...
			resp.error ? console.error('Emergency call log error:', resp.error) : console.log('Emergency call logged:', resp.data);
					...
					<View className="w-full max-w-md rounded-2xl bg-white p-6">
						...
						<View className="space-y-3">
							...
							<TouchableOpacity
								...
								className={`flex-row items-center rounded-xl border p-4 ${selectedCategories.quick
								...
								<View
									className={`h-6 w-6 rounded border-2 ${selectedCategories.quick ? 'border-blue-500 bg-blue-500' : 'border-gray-300'
							...
							<TouchableOpacity
								...
								className={`flex-row items-center rounded-xl border p-4 ${selectedCategories.emergency
								...
								<View
									className={`h-6 w-6 rounded border-2 ${selectedCategories.emergency ? 'border-red-500 bg-red-500' : 'border-gray-300'
										} items-center justify-center`}>
							...
							<TouchableOpacity
								...
								className={`flex-row items-center rounded-xl border p-4 ${selectedCategories.community
								...
								<View
									className={`h-6 w-6 rounded border-2 ${selectedCategories.community

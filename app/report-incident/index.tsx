...
export default function ReportIncidentIndex() {
	...
	const subcategories = {
		'Property Damage': ['Vandalism', 'Vehicle Damage', 'Building Damage', 'Equipment Damage', 'Other Property'],
		'Emergency Situation': ['Fire', 'Medical Emergency', 'Natural Disaster', 'Chemical Spill', 'Gas Leak'],
		'Crime in Progress': ['Theft/Robbery', 'Assault', 'Break-in', 'Domestic Violence', 'Drug Activity'],
		'Suspicious Activity': ['Suspicious Person', 'Suspicious Vehicle', 'Abandoned Item', 'Unknown Activity'],
		'Traffic Accident': ['Vehicle Collision', 'Pedestrian Accident', 'Motorcycle Accident', 'Bicycle Accident', 'Hit and Run'],
		'Public Disturbance': ['Noise Complaint', 'Public Fight', 'Harassment', 'Loitering', 'Disorderly Conduct'],
		'Other Incident': ['Found Property', 'Lost Pet', 'Utility Issue', 'Environmental Concern', 'Other']
	};
	...
	return (
		...
		>
			...
			<View className="bg-white border-b border-gray-200 px-6 py-3">
				<View className="flex-row items-center">
					...
					<View className="flex-row items-center mr-4">
						...
						<Text className="text-blue-600 font-medium text-xs ml-1">Basic Info</Text>
					</View>
					...
					<View className="flex-row items-center mr-4">
						...
						<Text className="text-gray-500 font-medium text-xs ml-1">Location & Details</Text>
					</View>
				...
				<View className="px-6">
					...
					>
						...
						<View>
								...
								<View>
									<Text className="text-gray-700 font-medium mb-2">Report Time</Text>
									<View className="bg-gray-100 border border-gray-300 rounded-lg px-4 py-3">
						...
						<Card className="mb-6">
							...
							<View className="space-y-4">
								{/* Trust Score */}
									...
									<Text className="text-gray-700 font-medium">Trust Score</Text>
									<View className="flex-row items-center space-x-2">
										<Text className="text-blue-600 font-bold text-lg">87%</Text>
										<View className="bg-gray-200 px-2 py-1 rounded">
											<Text className="text-gray-700 text-xs font-medium">VERIFIED</Text>
										</View>
										</View>
									</View>
								...
								{/* Report ID */}
								<View>
									<Text className="text-gray-700 font-medium mb-2">Report ID</Text>
									<View className="bg-gray-100 border border-gray-300 rounded-lg px-4 py-3">
										<Text className="text-gray-900 font-mono">RPT-959465</Text>
							</View>
						</View>
								...
								{/* Status */}
									...
									<View className="bg-green-100 px-3 py-1 rounded-full">
										<Text className="text-green-700 font-medium text-sm">Active Reporter</Text>
										</View>
								</View>
								...
								<View className="flex-row items-center justify-between">
									<View className="flex-row items-center">
										<View className="w-6 h-6 bg-gray-100 rounded items-center justify-center mr-3">
											<EyeOff size={16} color="#6B7280" />
														</View>
										<Text className="text-gray-700 font-medium">Submit as Anonymous Report</Text>
													</View>
									<TouchableOpacity
						...
						<ScrollView showsVerticalScrollIndicator={false}>
							{incidentCategories.map((category, index) => (
								...
								>
									<View className="flex-1">
										...
										<View className="flex-row items-center mt-1">
											<View className={`px-2 py-1 rounded-full ${
												category.severity === 'Critical' ? 'bg-red-100' :
												category.severity === 'High' ? 'bg-orange-100' :
												category.severity === 'Medium' ? 'bg-yellow-100' :
												'bg-gray-100'
											}`}>
												<Text className={`text-xs font-medium ${
													category.severity === 'Critical' ? 'text-red-700' :
													category.severity === 'High' ? 'text-orange-700' :
													category.severity === 'Medium' ? 'text-yellow-700' :
													'text-gray-700'
												}`}>
													{category.severity}
									</Text>
									...
									{incidentCategory === category.name && (
			...
			{showSubcategoryDropdown && incidentCategory && (
					...
					<View style={styles.dropdown}>
								<ScrollView showsVerticalScrollIndicator={false}>
							{subcategories[incidentCategory as keyof typeof subcategories]?.map((subcategory: string, index: number) => (
										<TouchableOpacity
									key={index}
										onPress={() => {
					...
					<View style={styles.dropdown}>
						<View className="flex-row">
							...
							<View className="flex-1 border-r border-gray-200">
								<Text className="text-center text-gray-600 font-medium py-2 border-b border-gray-200">Hour</Text>
								<ScrollView 
									...
									{hourOptions.map((hour, index) => (
										...
										>
											<Text className={`text-center ${
												selectedHour === hour ? 'text-blue-600 font-medium' : 'text-gray-900'
											}`}>
												{hour}
											</Text>
										</TouchableOpacity>
							...
							{/* Minutes */}
							<View className="flex-1 border-r border-gray-200">
								<Text className="text-center text-gray-600 font-medium py-2 border-b border-gray-200">Minute</Text>
								<ScrollView 
									showsVerticalScrollIndicator={true}
									style={{ height: 200 }}
									nestedScrollEnabled={true}
							...
							<TouchableOpacity
										...
										>
											<Text className={`text-center ${
												selectedMinute === minute ? 'text-blue-600 font-medium' : 'text-gray-900'
											}`}>
												{minute}
											</Text>
										</TouchableOpacity>
							...
							{/* AM/PM */}
							<View className="flex-1">
								<Text className="text-center text-gray-600 font-medium py-2 border-b border-gray-200">AM/PM</Text>
								<ScrollView 
									showsVerticalScrollIndicator={true}
									style={{ height: 200 }}
									nestedScrollEnabled={true}
								>
									{periodOptions.map((period, index) => (
										...
										>
											<Text className={`text-center ${
												selectedPeriod === period ? 'text-blue-600 font-medium' : 'text-gray-900'
											}`}>
												{period}
											</Text>
						</TouchableOpacity>
						...
						<TouchableOpacity
							...
							className={`mx-4 my-3 py-3 rounded-lg ${
